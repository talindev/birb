...
#define BIRB_TAB_STOP 8
#define BIRB_QUIT_TIMES 3
...
int editorRowCxToRx(erow *row, int cx) {
	...
	int j;
	for (j = 0; j < cx; j++) {
		if (row->chars[j] == '\t') rx += (BIRB_TAB_STOP - 1) - (rx % BIRB_TAB_STOP);
		rx++;
	}
	return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
	...
	for (cx = 0; cx < row->size; cx++) {
		if (row->chars[cx] == '\t') cur_rx += (BIRB_TAB_STOP - 1) - (cur_rx % BIRB_TAB_STOP);
		cur_rx++;
...
void editorUpdateRow(erow *row) {
	...
	free(row->render);
	row->render = malloc(row->size + tabs * (BIRB_TAB_STOP - 1) + 1);

	int idx = 0;
	for (j = 0; j < row->size; j++) {
		if (row->chars[j] == '\t') {
			row->render[idx++] = ' ';
			while (idx % BIRB_TAB_STOP != 0) row->render[idx++] = ' ';
		} else {
...
void editorProcessKeypress(void) {
	static int quit_times = BIRB_QUIT_TIMES;
